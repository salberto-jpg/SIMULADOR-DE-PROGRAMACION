...
export default function App() {
  ...
  const [swLength, setSwLength] = useState<string>(""); // Nueva casilla de longitud para ROTOLASER
  ...
  const handleCaptureTime = async () => {
    // Validación para ROTOLASER: si no es pincazo, longitud es obligatoria
        ...
        {!isSwExpanded ? (
          ...
          <div className="space-y-4 md:space-y-6">
            ...
            <div className="space-y-2.5 md:space-y-3">
              <select className="w-full bg-blue-900/50 p-3 rounded-xl text-[10px] font-bold uppercase border border-blue-800" value={swMachine} onChange={e => { setSwMachine(e.target.value); if(e.target.value === "ROTOLASER") setSwParam("rotolaser_corte"); }}>
                {machines.map(m => <option key={m.id} value={m.id}>{m.id}</option>)}
                {/* Asegurar que ROTOLASER esté si no está en INITIAL_MACHINES */}
              ...
              <select className="w-full bg-blue-900/50 p-3 rounded-xl text-[10px] font-bold uppercase border border-blue-800" value={swParam} onChange={e => setSwParam(e.target.value)}>
                {isRotolaser ? (
                  <>
                    <option value="rotolaser_corte">Velocidad de giro CORTE</option>
                    <option value="rotolaser_radio_chico">Velocidad de giro RADIO CHICO</option>
              ...
              {/* Casilla de longitud para ROTOLASER (excepto pincazo) */}
              {isRotolaser && swParam !== "rotolaser_pincazo" && (
                <div className="space-y-1 animate-in fade-in slide-in-from-top-2">
                  ...
                  <input 
                    type="number" 
                    className="w-full bg-blue-900/50 p-3 rounded-xl text-[11px] font-bold border border-blue-800 outline-none text-white focus:border-blue-300" 
                    value={swLength} 
                         ...
                         <table className="w-full text-left border-collapse">
                            <thead className="bg-slate-50 sticky top-0 z-10">
                               <tr className="border-b border-slate-100">
                                  ...
                                  <th className="px-6 md:px-8 py-3 md:py-4 text-[8px] md:text-[9px] font-black uppercase text-slate-400 text-right">Adjunto</th>
                               </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50">
                               {records.slice(0, 15).map(r => (
                                 <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 md:px-8 py-2 md:py-3 text-[9px] md:text-[10px] font-bold text-slate-400 text-right">{r.length ? `${r.length} mm` : '-'}</td>
                                 </tr>
                               ))}
                            </tbody>
                         </table>
