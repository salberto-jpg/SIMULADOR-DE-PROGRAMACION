...
const MachineProductionCard: React.FC<{ 
  ...
  return (
    <div className="bg-white rounded-[28px] border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[420px] md:h-[520px] hover:shadow-xl hover:border-blue-200 transition-all group">
      <div className="p-5 md:p-7 border-b border-slate-100 bg-slate-50/50">
        <div className="flex justify-between items-start mb-3">
          ...
          <span className={`text-[8px] md:text-[9px] font-black px-2.5 py-1 rounded-full uppercase ${occupancy > 90 ? 'bg-red-500 text-white' : occupancy > 70 ? 'bg-orange-500 text-white' : 'bg-blue-600 text-white'}`}>
            {occupancy.toFixed(0)}%
          </span>
        </div>
            ...
            <div key={batch.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:bg-white hover:border-blue-200 hover:shadow-md transition-all relative group/item">
              <div className="flex justify-between mb-1.5">
                <div className="flex items-center gap-2 truncate pr-2">
                  ...
                  <span className="text-[11px] font-black text-slate-900 uppercase truncate">{batch.name}</span>
                </div>
                <span className="text-[9px] font-black text-blue-600 whitespace-nowrap">{formatTime(batch.totalTime)}</span>
              </div>
              ...
              <div className="mt-3 flex gap-2 opacity-100 md:opacity-0 group-hover/item:opacity-100 transition-all">
                <button onClick={() => onEditBatch(batch)} className="text-[8px] md:text-[9px] font-black text-blue-600 uppercase bg-blue-50 px-2.5 py-1.5 rounded-lg border border-blue-100/50">Editar</button>
...
export default function App() {
  ...
  const handleCaptureTime = async () => {
    const value = swTime / 60; // Pasar a minutos decimales
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-['Inter'] relative overflow-hidden">
      ...
      <header className={`bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-40 px-6 md:px-12 py-5 transition-all duration-500 ${isSidebarOpen ? 'md:pl-80' : 'md:pl-24'}`}>
        <div className="max-w-[1600px] mx-auto flex justify-between items-center">
          <div className="flex items-center gap-6">
            ...
            >
              <div className="w-7 h-1 bg-slate-900 rounded-full group-hover:bg-blue-600 transition-all"></div>
              <div className="w-5 h-1 bg-slate-900 rounded-full group-hover:bg-blue-600 transition-all"></div>
              <div className="w-7 h-1 bg-slate-900 rounded-full group-hover:bg-blue-600 transition-all"></div>
            </button>
            <div>
      ...
      <aside 
        ...
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 text-white shadow-2xl transition-all duration-500 ease-in-out transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-[calc(100%-16px)]'} flex flex-col`}
      >
        ...
        <nav className="flex-1 py-8 px-4 space-y-2">
          {NAV_ITEMS.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all group ${activeTab === item.id ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}
            >
      ...
      {/* STOPWATCH FLOTANTE */}
      <div className={`fixed bottom-8 right-8 z-[60] bg-slate-900 text-white rounded-[32px] shadow-2xl transition-all duration-500 flex flex-col overflow-hidden ${isSwExpanded ? 'w-80 p-8' : 'w-20 h-20 items-center justify-center cursor-pointer hover:bg-blue-600'}`} onClick={() => !isSwExpanded && setIsSwExpanded(true)}>
        {!isSwExpanded ? (
          <span className="text-2xl">⏱️</span>
        ) : (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Captura de Tiempos</span>
              <button onClick={(e) => { e.stopPropagation(); setIsSwExpanded(false); }} className="text-slate-500 hover:text-white">&times;</button>
            </div>
            
            <div className="text-5xl font-black text-center tabular-nums py-2">{formatTime(swTime/60)}</div>
            
            <div className="space-y-3">
              <select className="w-full bg-slate-800 p-3 rounded-xl text-[10px] font-bold uppercase" value={swMachine} onChange={e => setSwMachine(e.target.value)}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button 
                ...
                className={`py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${swIsRunning ? 'bg-red-500' : 'bg-blue-600'}`}
              >
                {swIsRunning ? 'Pausar' : 'Iniciar'}
              </button>
              <button 
                onClick={handleCaptureTime}
                className="bg-green-600 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest"
              >
                Capturar
              </button>
            </div>
      ...
      <main className={`flex-1 p-6 md:p-12 transition-all duration-500 ${isSidebarOpen ? 'md:ml-72' : 'md:ml-12'}`}>
        <div className="max-w-[1600px] mx-auto w-full">
          ...
          {activeTab === 'schedule' && (
            <div className="space-y-10">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                 <div>
                    <h2 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tighter">Panel de Producción</h2>
                 ...
                 <div className="flex w-full md:w-auto gap-3">
                   <button 
                     ...
                     className="flex-1 md:flex-none bg-blue-600 text-white px-7 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl active:scale-95 transition-all"
                   >
                     + Nueva Simulación
                   </button>
                   ...
                   }} className="flex-1 md:flex-none bg-slate-900 text-white px-7 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all">Optimizar con IA</button>
                 </div>
              </div>
          ...
          {activeTab === 'machines' && (
            <div className="space-y-10">
              <h2 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tighter">Gestión de Máquinas</h2>
                ...
                {machines.map(m => (
                  <div key={m.id} className="bg-white p-10 rounded-[48px] border border-slate-200 shadow-sm hover:shadow-2xl transition-all group">
                    <div className="flex justify-between items-start mb-8">
                      <div>
                        <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">{m.id}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                      <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        ...
                        <span className="text-sm font-black text-slate-900">{batches.filter(b => b.machineId === m.id).length} Lotes</span>
                      </div>
                      <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                        <span className="text-[8px] font-black text-slate-400 uppercase block mb-1">Eficiencia</span>
                        <span className="text-sm font-black text-blue-600">{m.efficiency}%</span>
                      </div>
                    </div>
                    <button onClick={() => setIsEditing({ type: 'machine', data: m })} className="w-full py-5 bg-slate-900 text-white rounded-[24px] text-[11px] font-black uppercase tracking-[0.2em] shadow-lg group-hover:bg-blue-600 transition-all active:scale-95">Parámetros Técnicos</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* VISTA DE HERRAMENTAL */}
          {activeTab === 'tools' && (
            <div className="space-y-10">
              <div className="flex justify-between items-end">
                <div>
                  <h2 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tighter">Catálogo de Herramental</h2>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Inventario de punzones y matrices</p>
                </div>
                <button onClick={() => setIsEditing({ type: 'tool', data: { id: `t-${Date.now()}`, name: '', type: 'punch', angle: 88, maxTons: 100, length: 835, compatibleMachineIds: [] } })} className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest">Añadir Herramienta</button>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {tools.map(t => (
                  <div key={t.id} className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm hover:border-blue-300 transition-all">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <button onClick={() => deleteTool(t.id).then(loadData)} className="text-red-300 hover:text-red-500 text-sm">Eliminar</button>
                    </div>
                    <h4 className="text-sm font-black text-slate-900 uppercase mb-2">{t.name}</h4>
                    <div className="space-y-1.5 text-[10px] font-bold text-slate-500 uppercase">
                      <p>Ángulo: {t.angle}°</p>
                      <p>Largo: {t.length}mm</p>
                      <p>Carga Máx: {t.maxTons}T</p>
                      {t.vWidth && <p>Ancho V: {t.vWidth}mm</p>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'thickness' && (
            <div className="space-y-10">
              <div className="flex justify-between items-end">
                <div>
                  <h2 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tighter">Matriz de Espesores</h2>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Configuración técnica por material</p>
                </div>
                <button onClick={() => setIsEditing({ type: 'thickness', data: { id: `th-${Date.now()}`, value: 1.5, material: 'Hierro', recommendedToolIds: [] } })} className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest">Nuevo Espesor</button>
              </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {thicknesses.map(th => (
                      <tr key={th.id} className="hover:bg-slate-50/50 transition-all">
                        <td className="p-6 text-sm font-black text-slate-900">{th.value} mm</td>
                        ...
                        <td className="p-6">
                          <div className="flex flex-wrap gap-2">
                            {th.recommendedToolIds.map(tid => {
                              ...
                              return t ? <span key={tid} className="text-[8px] font-black bg-slate-100 text-slate-600 px-2 py-1 rounded-full">{t.name}</span> : null;
                            })}
                          </div>
                        </td>
                        <td className="p-6 text-right">
                          <button onClick={() => deleteThickness(th.id).then(loadData)} className="text-red-500 font-black text-[9px] uppercase tracking-widest">Eliminar</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {activeTab === 'records' && (
            <div className="space-y-10">
              <h2 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tighter">Historial de Tiempos</h2>
              <div className="bg-white rounded-[32px] border border-slate-200 overflow-hidden">
                <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b border-slate-100">
                    <tr>
                      <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Fecha</th>
                      <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Máquina</th>
                      <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Parámetro</th>
                      <th className="p-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Valor Medido</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {records.map(r => (
                      <tr key={r.id}>
                        <td className="p-6 text-xs text-slate-500 font-medium">{new Date(r.timestamp).toLocaleString()}</td>
                        <td className="p-6 text-sm font-black text-slate-900">{r.machineId}</td>
                        <td className="p-6 text-xs font-bold text-blue-600 uppercase">{r.parameter}</td>
                        <td className="p-6 text-sm font-black text-slate-900">{formatTime(r.value)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {/* MODAL SYSTEM */}
      {isEditing && (
        <div className="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-md flex items-end md:items-center justify-center p-0 md:p-6">
          <div className={`bg-white w-full ${isEditing.type === 'machine' ? 'max-w-6xl' : 'max-w-4xl'} rounded-t-[48px] md:rounded-[56px] shadow-2xl flex flex-col max-h-[96vh] overflow-hidden animate-in slide-in-from-bottom-full duration-500`}>
            
            <div className="px-10 py-10 border-b flex justify-between items-center bg-slate-900 text-white">
               <h3 className="text-2xl font-black uppercase tracking-tight">
                 {isEditing.type === 'batch' ? 'Configurar Lote' : 
                  isEditing.type === 'machine' ? `Ajustes: ${isEditing.data.id}` :
                  isEditing.type === 'tool' ? 'Nueva Herramienta' : 'Nuevo Espesor'}
               </h3>
               <button onClick={() => setIsEditing(null)} className="w-14 h-14 flex items-center justify-center rounded-full bg-slate-800 text-3xl hover:bg-red-500 transition-all">&times;</button>
            </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <div className="space-y-8">
                       <div className="group">
                         ...
                         <input className="w-full bg-slate-50 border-2 border-slate-100 p-6 rounded-[28px] font-bold text-xl outline-none focus:border-blue-500 transition-all" value={isEditing.data.name} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, name: e.target.value}})} placeholder="Ej: Soporte Frontal X1" />
                       </div>
                       ...
                       <div className="flex items-center justify-between p-6 bg-amber-50 rounded-[28px] border-2 border-amber-100">
                         ...
                         <input type="checkbox" className="w-8 h-8 rounded-xl" checked={isEditing.data.isSimulation} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, isSimulation: e.target.checked}})} />
                       </div>

                       <div className="grid grid-cols-2 gap-6">
                         <div>
                           <label className="text-[11px] font-black uppercase text-slate-400 mb-2.5 block tracking-widest">Piezas</label>
                           <input type="number" className="w-full bg-slate-50 border-2 border-slate-100 p-6 rounded-[28px] font-bold text-xl outline-none" value={isEditing.data.pieces} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, pieces: Number(e.target.value)}})} />
                         </div>
                         <div>
                           <label className="text-[11px] font-black uppercase text-slate-400 mb-2.5 block tracking-widest">Golpes/Pz</label>
                           <input type="number" className="w-full bg-slate-50 border-2 border-slate-100 p-6 rounded-[28px] font-bold text-xl outline-none" value={isEditing.data.strikesPerPiece} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, strikesPerPiece: Number(e.target.value)}})} />
                         </div>
                       </div>
                    </div>

                    <div className="bg-slate-50 p-10 rounded-[48px] border border-slate-200 space-y-8">
                       <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Configuración Logística</h4>
                       <div className="space-y-4">
                         <div className="flex items-center justify-between">
                            <span className="text-[11px] font-black text-slate-700 uppercase">Requiere Grúa (Volteo)</span>
                            <input type="checkbox" className="w-6 h-6" checked={isEditing.data.useCraneTurn} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, useCraneTurn: e.target.checked}})} />
                         </div>
                         <div className="flex items-center justify-between">
                            <span className="text-[11px] font-black text-slate-700 uppercase">Requiere Grúa (Giro)</span>
                            <input type="checkbox" className="w-6 h-6" checked={isEditing.data.useCraneRotate} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, useCraneRotate: e.target.checked}})} />
                         </div>
                       </div>
                       <div className="pt-4 border-t border-slate-200">
                         <label className="text-[11px] font-black text-slate-400 uppercase block mb-3">Máquina Asignada</label>
                         <select className="w-full p-5 rounded-2xl bg-white border border-slate-200 font-bold" value={isEditing.data.machineId} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, machineId: e.target.value}})}>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                   {[
                     ...
                     <div key={f.key} className="bg-slate-50 p-6 rounded-[32px] border border-slate-100">
                        ...
                        <input 
                          type="text" 
                          className="w-full bg-white border border-slate-200 p-5 rounded-[20px] font-black text-xl outline-none" 
                          value={f.key === 'efficiency' || f.key === 'productiveHours' || f.key === 'maxLength' || f.key === 'maxTons' ? isEditing.data[f.key] : formatTime(isEditing.data[f.key])} 
                          onChange={e => {
                            const val = e.target.value;
                            const newVal = f.key === 'efficiency' || f.key === 'productiveHours' || f.key === 'maxLength' || f.key === 'maxTons' ? Number(val) : parseTimeToMinutes(val);
               ...
               {isEditing.type === 'tool' && (
                 <div className="grid grid-cols-2 gap-8">
                    <div className="col-span-2">
                      ...
                      <input className="w-full p-6 bg-slate-50 rounded-2xl font-bold" value={isEditing.data.name} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, name: e.target.value}})} />
                    </div>
                    <div>
                      <label className="text-[11px] font-black uppercase text-slate-400 mb-2 block">Tipo</label>
                      <select className="w-full p-6 bg-slate-50 rounded-2xl font-bold" value={isEditing.data.type} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, type: e.target.value as any}})}>
                    ...
                    <div>
                      ...
                      <input type="number" className="w-full p-6 bg-slate-50 rounded-2xl font-bold" value={isEditing.data.angle} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, angle: Number(e.target.value)}})} />
                    </div>
                 </div>
               )}
                 ...
                 <div className="space-y-8">
                    <div className="grid grid-cols-2 gap-8">
                      <div>
                        ...
                        <input type="number" step="0.1" className="w-full p-6 bg-slate-50 rounded-2xl font-bold" value={isEditing.data.value} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, value: Number(e.target.value)}})} />
                      </div>
                      <div>
                        <label className="text-[11px] font-black uppercase text-slate-400 mb-2 block">Material</label>
                        <input className="w-full p-6 bg-slate-50 rounded-2xl font-bold" value={isEditing.data.material} onChange={e => setIsEditing({...isEditing, data: {...isEditing.data, material: e.target.value}})} />
                      </div>
                    </div>
                    <div>
                      <label className="text-[11px] font-black uppercase text-slate-400 mb-4 block">Herramientas Recomendadas</label>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {tools.map(t => (
                          <label key={t.id} className={`p-4 rounded-xl border-2 flex items-center gap-3 cursor-pointer transition-all ${isEditing.data.recommendedToolIds.includes(t.id) ? 'border-blue-500 bg-blue-50' : 'border-slate-100 bg-slate-50'}`}>
                            <input type="checkbox" className="w-5 h-5 rounded" checked={isEditing.data.recommendedToolIds.includes(t.id)} onChange={e => {
            ...
            <div className="px-12 py-12 bg-slate-50 border-t border-slate-200 flex flex-col md:flex-row gap-6">
               ...
               <button 
                 ...
                 className="order-1 md:order-2 flex-[2] bg-blue-600 text-white py-6 rounded-[32px] font-black text-[13px] uppercase tracking-[0.3em] shadow-2xl shadow-blue-500/40"
               >
                 Sincronizar Cambios
               </button>
            </div>
